...
import { summarizeDocuments, summarizeSubmittals } from "@/lib/projects/dashboard";
import type { AgentRun, Document, Issue, Submittal } from "@/lib/types/database";
...
export default async function ProjectOverviewPage({
  ...
  const issueSummary = summarizeIssueWorkflows(issues);
  const submittalSummary = summarizeSubmittals(submittals);

  const recentDocuments = documents.slice(0, 3);
  const recentIssues = issues.slice(0, 3);
  const recentSubmittals = submittals.slice(0, 3);
        ...
        <Card>
          ...
          <div className="mt-3 space-y-2">
            {recentIssues.length > 0 ? (
              ...
              <p className="text-sm text-zinc-500">No RFI issues yet.</p>
            )}
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="mt-3 space-y-2">
            {recentSubmittals.length > 0 ? (
              ...
              <p className="text-sm text-zinc-500">No submittals uploaded yet.</p>
            )}
          </div>
        </Card>
      </section>
